                                                   ...
                                                   const fs::path& tmpDir)
{
    ...
    for (const auto& [path, services] : result)
    {
        if (services.empty())
            continue;
                           ...
                           const fs::path& tmpDir)
{
    ...
    for (int i = 0; i < 256; ++i)
    {
        ...
        char payloadHex[4];
        snprintf(payloadHex, sizeof(payloadHex), "%02x", segmentId);
                                    ...
                                    const fs::path& tmpDir)
{
                               ...
                               componentIndex))
    {
        triedDiscovery = true;
        char payloadHex[16];
        snprintf(payloadHex, sizeof(payloadHex), "%02x%02x%02x%02x%02x",
                 componentClass & 0xFF, (componentClass >> 8) & 0xFF,
                 componentId & 0xFF, (componentId >> 8) & 0xFF, componentIndex);
    ...
    if (!success)
    {
        for (int cid = 0; cid <= 255; ++cid)
        {
            char payloadHex[16];
            snprintf(payloadHex, sizeof(payloadHex), "%02x%02x%02x%02x%02x",
                     NSM_FW_COMPONENT_CLASS & 0xFF,
                     (NSM_FW_COMPONENT_CLASS >> 8) & 0xFF, cid & 0xFF,
                     (cid >> 8) & 0xFF, 0);
